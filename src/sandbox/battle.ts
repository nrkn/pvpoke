...
var pokedex = gm.data.pokemon as any[]

var firstFive = pokedex.slice( 0, 5 )

for ( var i = 0; i < firstFive.length; i++ ) {

  if ( firstFive[ i ].fastMoves.length > 0 ) { // Only add Pokemon that have move data
    var pokemon = new Pokemon( firstFive[ i ].speciesId, 0, battle );
