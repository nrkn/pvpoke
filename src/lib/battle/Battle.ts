...
export function Battle(){
	...
	var self = this;
	var pokemon: any = [null, null];
	var players: any[] = [];
	var cp = 1500;
	var cup = {name: "all", include: [], exclude: []}; // List of allowed types

	var decisionLog: any[] = []; // For debugging
	var debug = false;

	var actions: any[] = []; // User defined actions
	var previousTurnActions: any[] = [] // Actions from the previous turn
	var turnMessages: any[] = []; // Array of messages to be displayed by the emulator for specific Pokemon
	var turnAnimations: any[] = []; // Animations to be displayed by the front end this turn
	var turnActions: any[] = []; // Actions to be performed this turn
	var queuedActions: any[] = []; // Input registered from previous turns to be processed on future turns
	var sandbox = false; // Is this automated or following user instructions?
	var mode = "simulate"; // Simulate or emulate?
	var chargedMoveUsed = false;
