...
import { webRoot, siteVersion } from "../app/settings";
import { Battle } from "./battle/Battle";
import { Pokemon } from "./pokemon/Pokemon";

// JavaScript Document

export var GameMaster = ( function() {
	var instance;
	...
	function createInstance() {
		...
		object.getPokemonById = function( id ) {
			...
			object.data.pokemon.forEach( function( poke, index ) {
		...
		object.getMoveById = function( id ) {
			...
			object.data.moves.forEach( function( m, index ) {
				if ( m.moveId == id ) {
		...
		object.getCupById = function( id ) {
			...
			object.data.cups.forEach( function( c, index ) {
				if ( c.name == id ) {
					cup = c;
				}
			} );
		...
		object.generateFilteredPokemonList = function( battle, include: boolean, exclude, rankingData, overrides ) {
		...
		object.generatePokemonListFromSearchString = function( str ) {
			...
			var results: any[] = []; // Store an array of qualifying Pokemon ID's
