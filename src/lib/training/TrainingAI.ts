...
import { webRoot } from "../../app/settings";
import { DecisionOption } from "./DecisionOption";
import { Battle } from "../battle/Battle";
import { TimelineAction } from "../battle/TimelineAction";
...
export function TrainingAI(l, p, b){
	...
	this.generateRoster = function(size, callback){
		...
		var roster: any[] = [];
		var selectedIds: any[] = []; // Array of Pokemon ID's to check to avoid duplicates
	...
	this.runScenario = function(type, pokemon, opponent){
		...
		for(var i = 0; i <= startStats[0].shields; i++){
			for(var n = 0; n <= startStats[1].shields; n++){
