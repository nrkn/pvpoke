...
export function Pokemon( id, pokedexIndex, b ) {
	...
	this.maximizeStat = function( sortStat ) {
		var combinations = self.generateIVCombinations( sortStat, 1, 1 );
	...
	this.generateIVCombinations = function( sortStat, sortDirection, resultCount, filters ) {
		...
		var results = combinations.splice( 0, resultCount );
	...
	*/
	...
	function getTypeEffectivenessArray( battle ) {
		...
		for ( var n = 0; n < allTypes.length; n++ ) {
			var effectiveness = battle.getEffectiveness( allTypes[ n ], self.types );
